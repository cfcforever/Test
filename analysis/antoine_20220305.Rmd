...
for (k in 1:nt){
  ...
  t1 = t2-1
  dat2 = data2[(t2-5):(t2+5),]

  dat1 = data1 %>%
    filter(record_timestamp>=data2[t1,"record_timestamp"]) %>%
    filter(record_timestamp<=data2[t2,"record_timestamp"])
  ...
  if (nrow(dat1)==0){
    ...
    q <- p + theme_bw() +
      ...
      geom_point(data = data2[(t1),], aes(x=x, y=y, col="Antoine before"), na.rm = T) +
  ...
  dat = dat[order(dat$record_timestamp),]
  print(tagList(datatable(dat[,c("tag","x","y","record_timestamp","date","diff_ts","diff_dist")], options = list(pageLength = 10))))

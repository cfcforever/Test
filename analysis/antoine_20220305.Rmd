...
for (k in 1:nt){
  ...
  if (nrow(dat1)==0){
    ...
    q <- p + theme_bw() +
      ...
      geom_point(data = data2[(t2-1):(t2),], aes(x=x, y=y, col="Antoine"), na.rm = T) +
  ...
  print(tagList(datatable(dat2[,c("tag","x","y","date","diff_ts","diff_dist")], options = list(pageLength = nrow(dat2)))))

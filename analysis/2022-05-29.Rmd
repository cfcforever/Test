...
dat <- data.frame(tag = unlist(lapply(json_data, function(x){x["tag_id"][[1]]})),
                  x = unlist(lapply(json_data, function(x){
                    output = x["x"][[1]]
                    output[is.null(output)] = NaN
                    return(output)
                  })),
                  y = unlist(lapply(json_data, function(x){
                    output = x["y"][[1]]
                    output[is.null(output)] = NaN
                    return(output)
                  })),
...
names_tag = names_tag[order(names_tag$Identifiant),]
```

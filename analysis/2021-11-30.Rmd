...
```{r}
nq = 10
table_sensibility = matrix(NA, nrow = nb_tag, ncol = nq+1)
colnames(table_sensibility) = paste0(c(0:10)/10*100, "%")
...
for (k in 1:nb_tag){
  ...
  table_sensibility[k,] = matrix(round(quantile(nd, 0:10/10),3), nrow = 1, ncol = nq+1)
}
...
date_sel = rev(as.Date(0:(-8), origin = "2021-11-30"))
